...
void consola(char* buffer, int socket_conexion) {
    ...
    if (string_starts_with(buffer, "GUARDAR_PAGINA")) { // Ejemplo: GUARDAR_PAGINA PROCESO NUMERO_PAGINA CONTENIDO
    ...
    if (string_starts_with(buffer, "OBTENER_PAGINA")) { // Ejemplo: OBTENER_PAGINA PROCESO NUMERO_PAGINA CONTENIDO
        if (cantidad_de_parametros == 4)
        {
            // Funcion que envia a Memoria el contenido de la pagina solicitada
        }
